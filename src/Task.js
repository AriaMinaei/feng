...
// function fnToSafeFn(computation) {
//   try {
//     computation();
//   } catch (e) {
  ...
  return new Task(function(reject, resolve) {
    return fork(function(a) {
      ...
      if (typeof result.fork !== 'function') {
        throw Error("Chained function does not return a Task.");
      }

      return result.fork(reject, resolve);
    });
  }, cleanup);
};

Task.prototype.cataChain = function _branch(pattern) {
  var fork = this.fork;
  var cleanup = this.cleanup;

  return new Task(function(reject, resolve) {
    return fork(function(a) {
      return pattern.Rejected(a).fork(reject, resolve);
    }, function(b) {
      return pattern.Resolved(b).fork(reject, resolve);
    });
  }, cleanup);
};
